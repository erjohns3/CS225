...
class kd_tree
{
  ...
  private:
     ...
     */
     ...
     void quick_select(size_t left, size_t right, size_t median, int dimension);

     size_t partition(size_t left, size_t right, int dimension);

     void build(size_t left, size_t right, int dimension);

     size_t find_nearest_neighbor(const point<Dim>& query, size_t left, size_t right, int dimension) const;

     double distance(const point<Dim> & lhs, const point<Dim> & rhs) const;
};
