...
maze::maze(uint64_t width, uint64_t height) : width_{width}, height_{height}
{
    ...
    for(uint64_t i=0; i<width_*height_; i++)
        ...
        if(i%width_ > 0 && i%width_ < width_){
            graph_.add_edge(vertices[i-1], vertices[i]);
            graph_.edge_weight(vertices[i-1], vertices[i], rng());
            graph_.edge_label(vertices[i-1], vertices[i], tools::edge_state::UNEXPLORED);

        }
        if(i/width_ > 0 && i/width_ < height_){
            graph_.add_edge(vertices[i-width_], vertices[i]);
            graph_.edge_weight(vertices[i-width_], vertices[i], rng());
            graph_.edge_label(vertices[i-width_], vertices[i], tools::edge_state::UNEXPLORED);
        }
    }
...
std::vector<direction> maze::solve()
{
    ...
    while(!q.empty()){
        u = q.front();
        q.pop();
        if(vtxID[u] >= width_*(height_-1))
            depths[vtxID[u]%width_] = q.size();
    ...
    for(uint64_t i=depth; i>0; i--){
        if(vtxID[parent[u]] < vtxID[u]-1)
            dirs[i] == direction::DOWN;
        else if(vtxID[parent[u]] == vtxID[u]+1)
            dirs[i] == direction::LEFT;
        else if(vtxID[parent[u]] > vtxID[u]+1)
            dirs[i] == direction::UP;
        else if(vtxID[parent[u]] == vtxID[u]-1)
