...
namespace cs225
{
	...
	:size_{0}, cap_{0}, front_{0}, back_{0}
	{
		/// @todo mp_parse.1
		try{
			arr_.reset(new T[cap_]);
		}catch (std::bad_alloc& e){}
	}

	template <class T>
	circ_array<T>::circ_array(uint64_t size)
	:size_{size}, cap_{size}, front_{0}, back_{size-1}
	{
		/// @todo mp_parse.1
		try{
			arr_.reset(new T[cap_]);
	...
	:size_{other.size_}, cap_{other.cap_}, front_{other.front_}, back_{other.back_}
	{
		/// @todo mp_parse.1
		try{
			arr_.reset(new T[cap_]);
