...
void lp_hash_table<K, V>::insert(K key, V value)
{
    ...
    for(auto i=0;i<size_;i++){
        if(states_[(i+idx)%size_] == occupancy::OCCUPIED && table_[(i+idx)%size_].first == key)
            remove(key);
        if(states_[(i+idx)%size_] == occupancy::UNOCCUPIED || states_[(i+idx)%size_] == occupancy::REMOVED){
            table_[(i+idx)%size_].first = key;
            table_[(i+idx)%size_].second = value;
            states_[(i+idx)%size_] = occupancy::OCCUPIED;
            break;
        }
    }
}
