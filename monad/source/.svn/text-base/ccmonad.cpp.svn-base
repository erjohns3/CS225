...
void partial_compilation(const string& compiler, const vector<string>& options,
                         ...
                         bool force = false)
{
    ...
    if (result1 != 0)
    {
        ...
        for (size_t split_i = 0; split_i < splits.size(); split_i++)
        {
            ...
            args.push_back("-DMONAD_SPLIT_LINE_NUMBER="
                           + lexical_cast<string>(splits[split_i]));
            args.push_back("-o");
...
int main(int argc, const char* const* argv)
{
    ...
    if (!isCompileOnly) // not really using cc-monad, just linking (maybe)
    {
        ...
        vector<string> linkargs(argv + argstart, argv + argc);
        int8_t link_result = exec(compiler, linkargs);
        ...
        if (link_result != 0)
        {
            ...
            if (possibleInputs.size() > 0)
            {
                ...
                {
                    partial_compilation(compiler, vector<string>(),
                                        possibleInputs[i] + ".cpp",
                                        possibleInputs[i] + ".o");
                    linkingArgs.push_back(possibleInputs[i] + ".o");
                }
                link_result = exec(compiler, linkingArgs);
            }
        }
        return link_result;
    }
    ...
    for (int32_t i = argstart; i < argc; i++)
    {
        if (strlen(argv[i]) > 0)
        {
            if (argv[i][0] == '-')
    ...
    {
        cerr << "cc-monad: error: cc-monad can only take one input file when "
                "given an explicit output file" << endl;
        exit(1);
    }
