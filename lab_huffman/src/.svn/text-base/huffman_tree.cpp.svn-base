...
void huffman_tree::build_tree(const vector<frequency>& frequencies)
{
    ...
    */
    ...
    for(size_t i=0; i<frequencies.size() ; i++)
        single_queue.push(std::unique_ptr<node>{new node(frequencies[i])});
    while(single_queue.size() + merge_queue.size() > 1){
        std::unique_ptr<node> tmp {new node(0)};

        tmp->left = std::move(remove_smallest(single_queue, merge_queue));
        tmp->right = std::move(remove_smallest(single_queue, merge_queue));
        tmp->freq = tmp->left->freq.count() + tmp->right->freq.count();

        merge_queue.push(std::move(tmp));
    }
    ...
    std::queue<std::unique_ptr<node>>& merge_queue) -> std::unique_ptr<node>
{
    ...
    */
    ...
    if(merge_queue.size() > 0 && single_queue.size() > 0){
...
void huffman_tree::decode(std::stringstream& ss, binary_file_reader& bfile)
{
    /**
    * @todo Your code here!
    *
...
auto huffman_tree::read_tree(binary_file_reader& bfile) -> std::unique_ptr<node>
{
    ...
    */
    if(!bfile.has_bits())
        return NULL;
    if(bfile.next_bit()){
        std::unique_ptr<node> tmp {new node(frequency(bfile.next_byte(), 1))};
        return std::move(tmp);
    }else{
        std::unique_ptr<node> tmp {new node(0)};
        tmp->left = read_tree(bfile);
        tmp->right = read_tree(bfile);
        return std::move(tmp);
    }
}
